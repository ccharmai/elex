...
export default {
	...
	created() {
		...
		axios.post(`${this.$store.getters.api}/get.types/`)
			.then(res => {
				if (res.data.status == 'ok') this.items = res.data.types;
	...
	methods: {
		addItems() {
			...
			this.add.show = false;
			axios.post(`${this.$store.getters.api}/add.types/`, { name: name, description: description, token: this.$store.getters.getToken })
				.then(res => {
